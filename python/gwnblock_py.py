...
class gwnblock_py(gr.basic_block):
    ...
    def set_timeout_size(self, number_timeouts):
        ...
        '''
        for i in list(range(0, number_timeouts)):
    ...
    def set_timer_size(self, number_timers):
        ...
        '''
        for i in list(range(0, number_timers)):
    ...
    def write_out(self, py_msg, port_nr=None):
        '''Sends an event converted to PMT on an output port.

        @param py_msg: a Python object, e.g. a dictionary.
        ...
        if port_nr is None:        # output message in all ports
            for outport in self.ports_out:
                lock_obj.acquire()
                outport.post_message(py_msg)
                lock_obj.release()                
        else:
            lock_obj.acquire()
            self.ports_out[port_nr].post_message(py_msg)
            lock_obj.release()
        return
